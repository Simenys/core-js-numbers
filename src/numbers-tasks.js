...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const point1 = x2 - x1;
  const point2 = y2 - y1;
  const formula = Math.sqrt(point1 ** 2 + point2 ** 2);
  return formula;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const coordinate1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const coordinate2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const magnitude = x1 * x2 + y1 * y2;
  const rad = magnitude / (coordinate1 * coordinate2);
...
function roundToPowerOfTen(num, pow) {
  const powLvl = 10 ** pow;
  return Math.round(num / powLvl) * powLvl;
}
...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i <= index; i += 1) {
    fibonaciNum = firstNum + secondNum;
    firstNum = secondNum;
